...
public enum CompressionType {
    NONE(0, "none", 1.0f) {
        @Override
        public OutputStream wrapForOutput(ByteBufferOutputStream buffer, byte messageVersion, BufferSupplier bufferSupplier) {
            return buffer;
        }
    ...
    LZ4(3, "lz4", 1.0f) {
        ...
        public OutputStream wrapForOutput(ByteBufferOutputStream buffer, byte messageVersion, BufferSupplier bufferSupplier) {
            try {
                return new KafkaLZ4BlockOutputStream(buffer, bufferSupplier, messageVersion == RecordBatch.MAGIC_VALUE_V0);
            } catch (Throwable e) {
                throw new KafkaException(e);
            }
        }
    ...
    ZSTD(4, "zstd", 1.0f) {
        @Override
        public OutputStream wrapForOutput(ByteBufferOutputStream buffer, byte messageVersio, BufferSupplier bufferSupplier) {
            try {
     ...
     */
    public abstract OutputStream wrapForOutput(ByteBufferOutputStream bufferStream, byte messageVersion, BufferSupplier bufferSupplier);
