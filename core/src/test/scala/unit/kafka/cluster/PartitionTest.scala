...
class PartitionTest extends AbstractPartitionTest {
  ...
  def testMaybeReplaceCurrentWithFutureReplicaDifferentBaseOffsets(): Unit = {
    ...
    val builder = MemoryRecords.builder(buffer, BufferSupplier.NO_CACHING, RecordBatch.CURRENT_MAGIC_VALUE, CompressionType.NONE,
  ...
  def testDelayedFetchAfterAppendRecords(): Unit = {
    ...
    def createRecords(baseOffset: Long): MemoryRecords = {
      ...
      val builder = MemoryRecords.builder(
        buf, BufferSupplier.NO_CACHING, RecordBatch.CURRENT_MAGIC_VALUE, CompressionType.NONE, TimestampType.CREATE_TIME,
  ...
  def createRecords(records: Iterable[SimpleRecord], baseOffset: Long, partitionLeaderEpoch: Int = 0): MemoryRecords = {
    val buf = ByteBuffer.allocate(DefaultRecordBatch.sizeInBytes(records.asJava))
    val builder = MemoryRecords.builder(
      buf, BufferSupplier.NO_CACHING, RecordBatch.CURRENT_MAGIC_VALUE, CompressionType.NONE, TimestampType.LOG_APPEND_TIME,
  ...
  def createTransactionalRecords(records: Iterable[SimpleRecord],
    ...
    val buf = ByteBuffer.allocate(DefaultRecordBatch.sizeInBytes(records.asJava))
    val builder = MemoryRecords.builder(buf, BufferSupplier.NO_CACHING, CompressionType.NONE, baseOffset, producerId,
